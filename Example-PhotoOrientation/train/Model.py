...
class Model:
    ...
    def create_graph(cls, input, keep_prob):
        ...
        model, shape = cls.__relu_layer(shape, model)
        model, shape = cls.__maxpool_layer(shape, model, 2)
        model, shape = cls.__conv_layer(shape, model, 3, 3, 96)
        model, shape = cls.__relu_layer(shape, model)
        model, shape = cls.__maxpool_layer(shape, model, 2)
        model, shape = cls.__conv_layer(shape, model, 3, 3, 128)
        model, shape = cls.__relu_layer(shape, model)
        model, shape = cls.__maxpool_layer(shape, model, 2)
        model, shape = cls.__conv_layer(shape, model, 3, 3, 160)
        model, shape = cls.__relu_layer(shape, model)
        model, shape = cls.__maxpool_layer(shape, model, 2)
        model, shape = cls.__fully_connected_layer(shape, model, 320)
        model, shape = cls.__relu_layer(shape, model)
